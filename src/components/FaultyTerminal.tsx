...
export default function FaultyTerminal({
  ...
  const resolvedDpr = dpr ?? (typeof window === "undefined" ? 1 : Math.min(window.devicePixelRatio || 1, 2))
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      if (!inViewportRef.current || !tabVisibleRef.current) return
      if (time - lastFrameTimeRef.current < 1000 / 30) return
      lastFrameTimeRef.current = time
      if (pageLoadAnimation && loadAnimationStartRef.current === 0) loadAnimationStartRef.current = time
      ...
      if (pageLoadAnimation && loadAnimationStartRef.current > 0) {
        program.uniforms.uPageLoadProgress.value = Math.min((time - loadAnimationStartRef.current) / 2000, 1)
      }
